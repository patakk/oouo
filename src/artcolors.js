...
function saturatecol([r, g, b], sat){
    ...
    if(sat > 0.0){
        var mx = Math.max(Math.max(cin.r, cin.g), cin.b);
...
function mixcol(c1, c2, blend){
    var c1 = new Color(c1[0], c1[1], c1[2], 1);
    var c2 = new Color(c2[0], c2[1], c2[2], 1);
    var aa = new Color(
...
function brightencol([r, g, b], br){
    ...
    if(br > 0){
        var rez = mixcol([r, g, b, 1], [1,1,1,1], br);
        return rez;
    }

    if(br < 0){
        var rez = mixcol([r, g, b, 1], [0,0,0,1], -br);
        return rez;
    }
}
...
function mixsubcol(cc1, cc2, p){
    var c1 = new Color(cc1[0], cc1[1], cc1[2], 1);
    var c2 = new Color(cc2[0], cc2[1], cc2[2], 1);
    var c = invcol(c1);
    var d = invcol(c2);

    var f = new Color(
        Math.max(0, 1. - c.r - d.r),
        Math.max(0, 1. - c.g - d.g),
        Math.max(0, 1. - c.b - d.b),
        1.
    )
    f = [f.r, f.g, f.b, f.a];
    ...
    return mixcollin(mixcollin(cc1, cc2, p), f, cd);
}
